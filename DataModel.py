...
class DataModel:
    ...
    def __init__(self):
        ...
        self._wolBroadcastAddress = "255.255.255.255"  # Sublan containing server, with 255 at end
        ...
        self._temperatureTarget = 0.0  # Camera setpoint target temperature
    ...
    def restore_ephemeris(self, savedData : ()):
        assert(len(savedData) == 4)
        self._skyFieldTimeScale = savedData[0]
        self._ephemeris = savedData[2]
        self._skyFieldTimeScaleNeedsLoad = savedData[1]
        self._ephemerisNeedsLoad = savedData[3]

    # Getters and Setters
    # _locationName
    def get_location_name(self) -> str: return self._locationName
    ...
    # _timeZone
    def get_time_zone(self) -> int: return self._timeZone
    ...
    # _latitude
    def get_latitude(self) -> float: return self._latitude
    ...
    # _longitude
    def get_longitude(self) -> float: return self._longitude
    ...
    # _startDateType
    def get_start_date_type(self) -> str: return self._startDateType
    ...
    # _startTimeType
    def get_start_time_type(self) -> StartTime: return self._startTimeType
    ...
    # _givenStartDate
    def get_given_start_date(self) -> str: return self._givenStartDate
    ...
    # _givenStartTime
    def get_given_start_time(self) -> str: return self._givenStartTime
    ...
    # _endDateType
    def get_end_date_type(self) -> str: return self._endDateType
    ...
    # _endTimeType
    def get_end_time_type(self) -> EndTime: return self._endTimeType
    ...
    # _givenEndDate
    def get_given_end_date(self) -> str: return self._givenEndDate
    ...
    # _givenEndTime
    def get_given_end_time(self) -> str: return self._givenEndTime
    ...
    # _disconnectWhenDone
    def get_disconnect_when_done(self) -> bool: return self._disconnectWhenDone
    ...
    # _warmUpWhenDone
    def get_warm_up_when_done(self) -> bool: return self._warmUpWhenDone
    ...
    # _warmUpWhenDoneSecs
    def get_warm_up_when_done_secs(self) -> bool: return self._warmUpWhenDoneSecs

    def set_warm_up_when_done_secs(self, value: bool):
        # print(f"setWarmUpWhenDoneSecs({value})")
        self._warmUpWhenDoneSecs = value

    # _sendWakeOnLanBeforeStarting
    def get_send_wake_on_lan_before_starting(self) -> bool: return self._sendWakeOnLanBeforeStarting
    ...
    def get_send_wol_seconds_before(self) -> float: return self._sendWolSecondsBefore
    ...
    # _wolMacAddress
    def getWolMacAddress(self) -> str: return self._wolMacAddress
    def setWolMacAddress(self, value: str):
        # print(f"setWolMacAddress({value})")
        self._wolMacAddress = value

    # _wolBroadcastAddress
    def getWolBroadcastAddress(self) -> str: return self._wolBroadcastAddress
    ...
    # _netAddress
    def get_net_address(self) -> str: return self._netAddress
    ...
    # _portNumber
    def get_port_number(self) -> str: return self._portNumber
    ...
    def get_temperature_regulated(self) -> bool: return self._temperatureRegulated
    ...
    # _temperatureTarget
    def get_temperature_target(self) -> float: return self._temperatureTarget
    ...
    # _temperatureWithin
    def get_temperature_within(self) -> float: return self._temperatureWithin
    ...
    def get_temperature_settle_seconds(self) -> float: return self._temperatureSettleSeconds
    ...
    # _maxCoolingWaitTime
    def get_max_cooling_wait_time(self) -> float: return self._maxCoolingWaitTime
    ...
    def get_temperature_fail_retry_count(self) -> int: return self._temperatureFailRetryCount
    ...
    def get_temperature_fail_retry_delay_seconds(self) -> float: return self._temperatureFailRetryDelaySeconds
    ...
    def get_temperature_abort_on_rise(self) -> bool: return self._temperatureAbortOnRise
    ...
    def get_temperature_abort_rise_limit(self) -> float: return self._temperatureAbortRiseLimit
    ...
    # _savedFrameSets
    def get_saved_frame_sets(self) -> [FrameSet]: return self._savedFrameSets
    ...
    def get_auto_save_after_each_frame(self) -> bool: return self._autoSaveAfterEachFrame
    ...
    # Determine if we have enough information to allow the acquistion session to run
    ...
    def add_frame_set(self, new_frame_set : FrameSet):
    ...
    def insert_frame_set(self, new_frame_set : FrameSet, at_index : int):
    ...
    def calc_sunrise(self, end_date_type : str, given_end_date : date) -> time:
        ...
        # If sunrise has already occured and endtype is "TODAY_TOMORROW", we'll re-calculate it
        # for tomorrow.

        if (current_time > sunrise_local_time) and (end_date_type == EndDate.TODAY_TOMORROW):
            # print(f"   Sunrise {sunrise_local} has already occured, switching to tomorrow")
    ...
    def calc_sunrise_and_sunset(self, start_date_type : str, given_start_date : date) -> []:
    ...
    def get_location_topos(self, latitude : float, longitude : float) -> Topos:
    ...
    def get_session_time_info(self) -> SessionTimeInfo:
        ...
        if _end_date_type == EndDate.WHEN_DONE:
            ...
            if (end_time is not None) and (end_time > start_time):
                ...
                # assume they meant *tomorow* morning
    ...
    def appropriate_end_time(self):
        ...
        end_date = date.today()
        _end_date_type = self.get_end_date_type()
        _given_end_date_string: str = self.get_given_end_date()
        if _end_date_type == EndDate.GIVEN_DATE:
            end_date = DataModel.parse_date(_given_end_date_string)
        _end_time_type = self.get_end_time_type()
        if _end_time_type == EndTime.SUNRISE:
            if (self.get_latitude() != DataModel.LATITUDE_NULL) and (self.get_longitude() != DataModel.LONGITUDE_NULL):
                result = self.calc_sunrise(_end_date_type, end_date)
                # print("  Using sunrise")
        elif _end_time_type == EndTime.CIVIL_DAWN:
            if (self.get_latitude() != DataModel.LATITUDE_NULL) and (self.get_longitude() != DataModel.LONGITUDE_NULL):
                result = self.calc_civil_dawn(_end_date_type, end_date)
                # print("  Using civil dawn")
        elif _end_time_type == EndTime.NAUTICAL_DAWN:
            if (self.get_latitude() != DataModel.LATITUDE_NULL) and (self.get_longitude() != DataModel.LONGITUDE_NULL):
                result = self.calc_nautical_dawn(_end_date_type, end_date)
                # print("  Using nautical dawn")
        elif _end_time_type == EndTime.ASTRONOMICAL_DAWN:
            if (self.get_latitude() != DataModel.LATITUDE_NULL) and (self.get_longitude() != DataModel.LONGITUDE_NULL):
                result = self.calc_astronomical_dawn(_end_date_type, end_date)
                # print("  Using astronomical dawn")
        else:
            assert(_end_time_type == EndTime.GIVEN_TIME)
